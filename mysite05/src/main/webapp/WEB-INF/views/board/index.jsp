							...
							<c:choose>
								<c:when test="${vo.depth > 0 }">
									<td class="left"
										...
										<a
										href="${pageContext.request.contextPath }/board/view/${vo.id }?p=${map.currentPage }&kwd=${map.keyword }">${vo.title }</a>
									</td>
								</c:when>
								<c:otherwise>
							...
							<td><sec:authorize access="isAuthenticated()">
									...
									<c:choose>
										<c:when test="${authUser.id == vo.userId }">
											<a
												href="${pageContext.request.contextPath }/board/delete/${vo.id }?p=${map.currentPage }&kwd=${map.keyword }"
												class="del"
